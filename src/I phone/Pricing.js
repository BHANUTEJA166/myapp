...
// import { buyIphoneAndUpdateStock } from '../Redux/Iphones/IphoneActions';
...
//                     onClick={() => dispatch(buyIphoneAndUpdateStock(item.id))}
//                   >
//                     Buy Now
//                   </button>
//                 </div>
...
import {
  fetchStocks,
  buyIphoneAndUpdateStock
} from '../Redux/Iphones/IphoneActions';

const Pricing = ({ stocks, loading, error, fetchStocks, buyIphone }) => {
  const [products, setProducts] = useState([]);

  useEffect(() => {
    // ✅ STATIC API (Vercel safe)
    axios
      .get('/data/database.json')
      .then(res => setProducts(res.data.products))
      .catch(console.error);

    // fetch redux stocks once
    fetchStocks();
  }, [fetchStocks]);

  const getStock = name =>
    stocks.find(s => s.name === name)?.stock || 0;
      ...
      <div className="container">
        <div className="text-center mb-5">
          <h2 className="display-5 fw-bold">
            iPhone. Power that fits your life.
          </h2>
          <p className="lead text-muted mt-3">
            Choose the iPhone that’s right for you —<br />
            Same performance. Different sizes. Your style.
                ...
                <div className="card h-100 border-0 shadow-sm text-center">
                  ...
                  <div className="card-footer bg-white border-0">
                    <button
                      ...
                      onClick={() =>
                        buyIphone(
                          stocks.find(s => s.name === p.name)?.id
                        )
                      }
                    >
                      {stock > 10
