...
import { connect } from "react-redux";
import { fetchStocks } from "../../Redux/Iphones/IphoneActions";
import { loadEnquiries } from "../../Redux/Enquiries/EnquiryActions";
import useCallApi from "./CallApi";
...
const Welcome = ({
  ...
  // static counts (unchanged from your original design)
  const Features    = useCallApi("Features");
  const Accessories = useCallApi("Accessories");
  const Reviews     = useCallApi("Reviews");

  // load redux data once
  useEffect(() => {
    fetchStocks();
    loadEnquiries();
  }, [fetchStocks, loadEnquiries]);

  // total stock (same logic as original)
  const totalStock = stocks.reduce(
    (sum, item) => sum + item.stock,
    0
  );
  ...
  return (
    <div className="h-100 p-5 text-center">
      ...
      {/* 🔹 MODEL-WISE STOCK VIEW (THIS WAS MISSING) */}
      {!loading && !error && (
        <div className="row mt-5">
          {stocks.map(item => {
            const statusClass =
...
const mapDispatchToProps = dispatch => ({
  fetchStocks: () => dispatch(fetchStocks()),
  loadEnquiries: () => dispatch(loadEnquiries())
});
